...
UNIV_BASE_DIR = "/q/coredata/data/prod/na/1.2/univ/"
SECDATA_BASE_DIR = "/q/strat/shared/data/estu/na/5.1/"
PRICE_BASE_DIR = "/q/coredata/data/prod/na/1.2/prices/"
BARRA_BASE_DIR = "/q/coredata/data/prod/na/1.2/barra/"
BAR_BASE_DIR = "/q/work/sean/prod/data/qbars/"
EARNINGS_BASE_DIR = "/q/work/sean/prod/data/earnings/"
LOCATES_BASE_DIR = "/q/work/sean/prod/data/locates/"
# ESTIMATES_BASE_DIR = "/r/data_f/coredata/data/prod/na/1.2/estimates/"
ESTIMATES_BASE_DIR = "/q/work/sean/tmp/"
...
def get_uni(start, end, lookback, uni_size=1400):
    ...
    print
    "Universe size (secdata): {}".format(len(univ_df.index))
    univ_df = univ_df[(univ_df['estu_barra4s'] == 1) | (univ_df['ind'] == 404020)]
    del univ_df['estu_inter']
    del univ_df['estu_barra4s']
    print
    "Universe size (estu_inter): {}".format(len(univ_df.index))

    univ_df = univ_df[univ_df['group'] != 3520]
    print
    "Universe size (bio): {}".format(len(univ_df.index))

    price_dir = PRICE_BASE_DIR + year
    price_file = price_dir + "/" + unidate + ".equ_prices.rev.csv"
    ...
    while (date < end):
        ...
        try:
            bars_df = pd.read_csv(bar_file, compression='gzip', sep="|", header=None,
                                  ...
                                  converters={'iclose_ts': fromtimestamp}, usecols=cols)
        except IOError:
            print
        ...
        try:
            bars_df = pd.read_csv(bar_file, compression='gzip', sep="|", header=None,
                                  names=['ticker', 'bopen_ts', 'iclose_ts', 'bopen', 'bfirst', 'bhigh', 'blow', 'blast',
                                         ...
                                         'outsideDollars'],
                                  converters={'iclose_ts': fromtimestamp, 'bopen_ts': fromtimestamp}, na_values=['-1'])
        except IOError:
            print
            "File not found: {}".format(bar_file)
            date += timedelta(days=1)
...
def load_earnings_dates(uni_df, start, end):
    ...
    while (date < end):
        ...
        earnings_file = earnings_dir + "/" + dateStr + ".expected_earnings_dates.rev.csv"
        try:
...
def load_locates(uni_df, start, end):
    ...
    while (date < end):
        ...
        locates_file = locates_dir + dateStr + ".locates.rev.csv"
        try:
...
def load_qb_eods(mdir, start, end):
    ...
    "Looking in {}".format(mdir)
    fcast_dfs = list()
    files = sorted(glob.glob(mdir + "/*.EOD.csv"))
    for ff in files:
